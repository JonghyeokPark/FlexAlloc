...
int main(int argc, char **argv)
{
  ...
  ret = poolfs_pool_set_strp(pfsh, num_strps, OPT_IO_SIZE / num_strps);
  if (ret)
    goto out;
  ...
  {
    snprintf(obj_name, POOLFS_OBJ_NAME_LEN_MAX, "%s.%lu", POOL_NAME, count);
    ...
    while (to_write)
    {
      ...
      ret = poolfs_object_write(oh, obj_name, to_access - to_write, cur_io_sz, pfsh);
      if (ret)
      {
  ...
  for (count = 0; count < num_objs; count++)
  {
    ...
    while (to_read)
    {
      ...
      ret = poolfs_object_read(oh, obj_name, to_access - to_read, cur_io_sz, pfsh);
      if (ret != cur_io_sz)
      {
